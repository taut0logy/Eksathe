...
export default function ConversationItem({conversation, selectedConversation = null, online = null}) {
    ...
    if (selectedConversation) {
        if (!selectedConversation.is_server && !conversation.is_server &&
        ...
        >
            ...
            <div
                ...
                <div className="flex items-center gap-1 justify-between">
                    <h3 className={`text-sm font-semibold overflow-hidden text-nowrap text-ellipsis ${conversation.is_user && conversation.blocked_at ? 'opacity-60' : ''}`}>{conversation.name}</h3>
                    {
                ...
                </div>
                {
                    conversation.last_message && (
                        <p className={`text-nowrap text-xs overflow-hidden text-ellipsis ${conversation.is_user && conversation.blocked_at ? 'opacity-60' : ''}`}>{conversation.last_message}</p>
                    )
                }
            </div>
            {
                (user.is_admin && conversation.is_user) ?(<UserOptionsDropdown conversation={conversation}/>):null
            }
        </Link>
    )
}
